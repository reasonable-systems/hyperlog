...
const Protocol = function (opts) {
  ...
  const self = this
  const parse = through.obj(function (data, enc, cb) {
...
Protocol.prototype._encode = function (type, enc, data, cb) {
  const buf = Buffer.alloc(enc.encodingLength(data) + 1)
  buf[0] = type
  enc.encode(data, buf, 1)
  this._encoder.write(buf, cb)
}

const decodeMessage = function (data) {
  switch (data[0]) {
